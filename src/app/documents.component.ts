...
setRangeValues(){
    ...
    getTotalArray(){
    var start = 0, end =500, count =1, obj = [];
    ...
    onSubmitModal(){
        ...
        total = rangeValue1+rangeValue2;
        
        if(rangeValue1==0 && rangeValue2==0 ){
            total = 500;
        }else{
            total = total+500;
        }
        ...
        if (guids.length > 0) {            
            this.dataService.getResponse("https://pubdocdata.worldbank.org/PubDataSourceAPI/multidownload-stats", JSON.stringify({"stats": guids})).subscribe((response)=> {             
                ...
                Object.keys(documents).forEach((document) => {  
                    ...
                    let parentTopic= this.emptyCheck(doc,this.params.TOPICS);
                    parentTopic = parentTopic.replace(/m+[\d]+ [\d]+/g,"");
                    let abstract ="";
                    if(doc.abstracts!=undefined){
                    ...
                    documentsExcel.push({
                     ...
                     //  [this.i18nService[this.params.ABSTRACTS]]: doc[this.params.ABSTRACTS] == undefined ? "" : this.filter.transform(doc[this.params.AUTHORS]["cdata!"]),   
                        "Abstract":abstract,
                       //[this.i18nService[this.params.ABSTRACTS]]: this.emptyCheck(abstract, this.params.ABSTRACTS),
